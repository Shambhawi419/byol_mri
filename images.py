...
import sys, os
sys.path.insert(0, '/workspace/byol_mri')
from models.varnet import VarNet
from utils.fourier import ifft2c
from utils.math import complex_abs, rss
...
for fname, slice_id in targets:
    ...
    fig.suptitle(f'{fname} slice {slice_id} - 4x acceleration', fontsize=14)
    for ax, (title, img) in zip(axes, [
        ...
        ax.axis('off')
    plt.tight_layout()
    outname = f'/workspace/output_images/{fname.replace(".pt","")}_s{slice_id}.jpg'
